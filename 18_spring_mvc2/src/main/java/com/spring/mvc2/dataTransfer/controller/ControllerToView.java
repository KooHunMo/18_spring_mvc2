...
public class ControllerToView {
	 ...
	 *  예시 1) Model
	 *  
	 *  메서드의 파라미터로 Model을 추가하여 addAddtribute("속성명","값") 
	 *  메서드로 뷰에서 사용할 데이터를 전송 한다.
	 * 
	 * */
	 ...
	 * 예시 2) ModelAndView
	 * 
	 * ModelAndView객체를 생성하여  addObject("속성명", "값") 
	 * 메서드로 뷰에서 사용할 데이터를 전달 한다. 
	 * view도 하고 model 데이터도 데리고 다니는 것이다.
	 * 관용적으로 객체명으로 mv 혹은 mav로 작성한다.
	 * 
	 * */
	@RequestMapping(value="/modelAndViewEx", method=RequestMethod.GET)
	public ModelAndView medelAndViewEx() {  // return 타입은 String이 아닌 ModelAndView 클래스가 된다. 
											//ModelAndView가 객체가 되니까
		
		// ModelAndView mv = new ModelAndView(); 		// ModelAndView 객체 생성.
		// mv.setViewName("dataTransfer/memberList");   // setViewName() 메서드로 포워딩(넘길) 할 jsp 파일  생성.
	 ...
	 * 
	 * HttpServletRequest객체를 생성하여 setAttribute("속성명", "값") 메서드로 뷰에서 
	 * 사용할 데이터를 전달 한다.
	 * 
	 * */
	...
	/*
	 	...
	 	# 예시 4) ResponseBody //= html의 body를 응답한다// jsp의 화면을 가져오는 것이 
	 														아니라 html 을 만드는 것이다.
	...
	/*
		 ...
		 # 예시 5) ResponseEntity
			 ...
			 - HTTP에서 Request와 Response 동작시 전송되는 정보는 
			   크게 body(몸체) , headers(헤더), status code(상태 코드)가 있다.
	...
	*/
	...
	public ResponseEntity<Object> responseEntityEx() {
		...
		//return new ResponseEntity<Object>(HttpStatus.OK); // 굳이 변수에 담지 않고 이 객체를 
															// 바로 리턴으로 돌린다
										   //본문, 헤더, 상태코드
		...
		// 3)
		//return new ResponseEntity<Object>("<h1>html 페이지반환</h1>" , new HttpHeaders() 
																	// , HttpStatus.OK);
	
		HttpHeaders responseHeader = new HttpHeaders();
		responseHeader.add("Content-Type" , "text/html; charset=UTF-8"); // joinForm에서 헤더 뽑아서 
																		 // Content-Type으로 바꿔야함 
																		 //	,""도 붙이고
...
@RestController  //@RestController를 붙이면 무조건 html의 바디가 리턴된다. 
				 //한글처리가 어려워서 ResponseEntity를 강제로 써야하는 상황이 있을 수 있다.
